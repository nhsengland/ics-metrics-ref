...
# Set Parameters
knitr::opts_chunk$set(echo = params$analyst_mode, message=FALSE, warning = FALSE)
...
list.files('theme', '\\.[Rr]', full.names = T) %>% map(source)
list.files('code', '\\.[Rr]', full.names = T) %>% map(source)
closeAllConnections()
...
inds <- get_indicators()
# Load data
smd <- read_csv('data/final_data_lad.csv')
...
```{r intro}
md('intro.md')
sys_metrics_tab <- read_csv('data/system_metrics.csv')
